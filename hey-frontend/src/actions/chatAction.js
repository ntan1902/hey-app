...
import { changeTransferStatements, newTransferStatement } from "./paymentAction";
...
export function receivedChatList(chatList) {
    ...
    if (fetchedChatList.length > 0) {
        header = {
            title: fetchedChatList[0].groupName === "" ?
                fetchedChatList[0].name : fetchedChatList[0].groupName,
...
export function receivedNewMessage(message) {
    ...
    if (message.transferStatement) {
        PaymentAPI.getTransferStatement(0, 1).then((res) => {
