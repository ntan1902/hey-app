...
@AllArgsConstructor
@Log4j2
@CrossOrigin({ "http://localhost:3000", "http://web.oispyouthunion.vn", "https://web.oispyouthunion.vn" })
public class UserController {
    ...
    public ResponseEntity<ApiResponse> createPin(@RequestBody @Valid PinRequest pinRequest)
        ...
        return ResponseEntity.ok(ApiResponse.builder().success(true).code(HttpStatus.CREATED.value())
                .message("Create PIN successfully").payload("").build());
    }
    ...
    public ResponseEntity<ApiResponse> findUsernameByUserId(@PathVariable("userId") String userId)
            throws UserIdNotFoundException {
    ...
    @PostMapping("/logout")
    public ResponseEntity<ApiResponse> logout(@RequestBody @Valid LogOutRequest request)
    ...
    public ResponseEntity<ApiResponse> uploadImage(@RequestParam("file") MultipartFile multipartFile)
            throws IOException {
